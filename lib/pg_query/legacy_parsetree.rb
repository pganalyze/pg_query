  ...
  class ParseResult
    ...
    def parsetree # rubocop:disable Metrics/CyclomaticComplexity
      @parsetree ||= transform_nodes!(@tree) do |raw_node|
        ...
        when INDEX_STMT
          node[INDEX_STMT].delete('oldCreateSubid')
          node[INDEX_STMT].delete('oldFirstRelfilenodeSubid')
        when RANGE_VAR
          node[RANGE_VAR]['inhOpt'] = node[RANGE_VAR].delete('inh') ? 2 : 0
        when TYPE_NAME
          node[TYPE_NAME]['names'] = transform_string_list(node[TYPE_NAME]['names'])
        when SELECT_STMT
          node[SELECT_STMT].delete('limitOption') if node[SELECT_STMT]['limitOption'] == 0
        end
